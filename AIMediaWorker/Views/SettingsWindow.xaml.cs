...
public sealed partial class SettingsWindow : Window
{
    ...
    public Array HdrOutputModes { get; } = Enum.GetValues<HdrOutputMode>();
    public Array RtxModes { get; } = Enum.GetValues<RtxVideoSuperResolutionMode>();
    public IReadOnlyList<string> ToolbarIconSizeOptions { get; } = [L("ToolbarIconSizeDefault"), L("ToolbarIconSizeLarge")];
    ...
    private async void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        HardwareCombo.SelectedItem = _settings.Playback.HardwareDecoder; HdrOutputCombo.SelectedItem = _settings.Playback.HdrOutput; RtxCombo.SelectedItem = _settings.Playback.RtxVideoSuperResolution; VolumeBox.Value = _settings.Playback.DefaultVolume; SeekBox.Value = _settings.Playback.SeekIntervalSeconds; ToolbarIconSizeCombo.SelectedIndex = _settings.Playback.UseLargeToolbarIcons ? 1 : 0;
        ...
        if (!rtx.IsSupported) { RtxCombo.SelectedItem = RtxVideoSuperResolutionMode.Off; RtxCombo.IsEnabled = false; }
    ...
    private async void OnInstallAsrClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var workerDirectory = ResolveAsrWorkerDirectory(AsrWorkerFolderBox.Text);
            var progress = new Progress<AsrInstallationProgress>(UpdateAsrInstallationProgress);
            await new AsrInstallationService(workerDirectory).InstallAsync(progress, _asrInstallCancellation.Token);
    ...
    private async void OnSaveClick(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(CrispAsrRuntimeBox.Text)) throw new InvalidOperationException(L("CrispAsrRuntimeRequiredMessage"));
            ...
            _settings.General.Language = (AppLanguage)(LanguageCombo.SelectedItem ?? AppLanguage.Default); _settings.General.Theme = (AppTheme)(ThemeCombo.SelectedItem ?? AppTheme.Dark); _settings.General.UiFontFamily = UiFontButton.Content?.ToString() ?? GeneralSettings.DefaultUiFontFamily; _settings.General.RecentMediaCount = (int)RecentCountBox.Value; _settings.General.ResumePlayback = ResumeCheck.IsChecked == true; _settings.General.DefaultFolder = EmptyToNull(DefaultFolderBox.Text);
            _settings.Playback.HardwareDecoder = (HardwareDecoder)(HardwareCombo.SelectedItem ?? HardwareDecoder.Auto); _settings.Playback.HdrOutput = (HdrOutputMode)(HdrOutputCombo.SelectedItem ?? HdrOutputMode.Auto); _settings.Playback.RtxVideoSuperResolution = (RtxVideoSuperResolutionMode)(RtxCombo.SelectedItem ?? RtxVideoSuperResolutionMode.Auto); _settings.Playback.DefaultVolume = VolumeBox.Value; _settings.Playback.SeekIntervalSeconds = SeekBox.Value; _settings.Playback.UseLargeToolbarIcons = ToolbarIconSizeCombo.SelectedIndex == 1; _settings.Playback.Renderer = RendererBox.Text.Trim(); _settings.Playback.DefaultAudioLanguage = EmptyToNull(AudioLanguageBox.Text); _settings.Playback.DefaultSubtitleLanguage = EmptyToNull(SubtitleLanguageBox.Text);
    ...
    private async void OnBrowseCaptureFolderClick(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            InitializeWithWindow.Initialize(picker, WindowNative.GetWindowHandle(this));
            var folder = await picker.PickSingleFolderAsync();
            if (folder is not null) CaptureFolderBox.Text = folder.Path;
        }
        catch (Exception exception) { StatusBar.Title = L("SettingsErrorTitle"); StatusBar.Message = exception.Message; StatusBar.Severity = InfoBarSeverity.Error; StatusBar.IsOpen = true; }
    }

    private void OnClearCaptureFolderClick(object sender, RoutedEventArgs e) => CaptureFolderBox.Text = string.Empty;

    private async void OnBrowseAsrWorkerFolderClick(object sender, RoutedEventArgs e)
    {
        try
        {
            var picker = new FolderPicker { SuggestedStartLocation = PickerLocationId.DocumentsLibrary };
            ...
            InitializeWithWindow.Initialize(picker, WindowNative.GetWindowHandle(this));
            var folder = await picker.PickSingleFolderAsync();
            if (folder is not null) AsrWorkerFolderBox.Text = folder.Path;
        }
        catch (Exception exception) { StatusBar.Title = L("SettingsErrorTitle"); StatusBar.Message = exception.Message; StatusBar.Severity = InfoBarSeverity.Error; StatusBar.IsOpen = true; }
    }

    private void OnClearAsrWorkerFolderClick(object sender, RoutedEventArgs e) => AsrWorkerFolderBox.Text = string.Empty;

    private void OnAsrWorkerFolderChanged(object sender, TextChangedEventArgs e)
    {
        try
        {
    ...
    private static string ResolveAsrWorkerDirectory(string text)
    {
        try { return AsrRuntimePaths.ResolveWorkerDirectory(text); }
        catch (Exception exception) when (exception is ArgumentException or NotSupportedException or System.Security.SecurityException or IOException)
        {
            throw new InvalidOperationException(L("AsrWorkerFolderInvalidMessage"), exception);
        }
    }
